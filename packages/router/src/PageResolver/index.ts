...
export class PageResolver implements IPageResolver {
  ...
  addLocalResolver(locale: string, config: object) {
  ...
  resolve(locale: string, path: string, queryParams: Record<string, string>) {
  ...
  mapRule(rule, params: Record<string, string>) {
    const paramsKeys = Object.keys(params);
    return paramsKeys.reduce(
      (res, name) => {
